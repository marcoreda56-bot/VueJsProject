<template>
  <div class="p-6 max-w-4xl mx-auto">
    ...
    >
      ...
      <p class="text-gray-500 dark:text-slate-400 font-bold">No notifications yet.</p>
    </div>
    ...
    <div v-else class="space-y-3">
      <div
        ...
        :key="notif.id"
        class="bg-white dark:bg-slate-900 p-5 rounded-xl shadow-sm border dark:border-slate-800 flex items-start gap-4 cursor-pointer hover:shadow-md transition-all"
        :class="{ 'border-l-4 border-l-indigo-500': !notif.is_read }"
        @click="handleClick(notif)"
      >
        <div
          class="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
          :class="notif.is_read ? 'bg-slate-100 dark:bg-slate-800 text-slate-400' : 'bg-indigo-100 dark:bg-indigo-900/20 text-indigo-600'"
        >
