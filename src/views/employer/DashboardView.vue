  ...
  <div class="space-y-10">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      <div v-for="stat in stats" :key="stat.label"
        class="bg-white dark:bg-slate-900 p-8 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm transition-all group hover:border-indigo-200 cursor-pointer"
        @click="stat.onClick ? stat.onClick() : null"
      >
        <div :class="stat.color" class="w-12 h-12 rounded-2xl flex items-center justify-center text-white mb-6 group-hover:scale-110 transition-transform">
          <i :class="stat.icon" class="text-xl"></i>
        </div>
        <p class="text-slate-400 font-black text-[10px] uppercase tracking-[0.2em] mb-1">{{ stat.label }}</p>
        <p class="text-3xl font-black text-slate-900 dark:text-white">{{ employerStore.loading ? '...' : stat.value }}</p>
        <p v-if="stat.sublabel" class="text-[10px] text-slate-400 font-bold mt-2">{{ stat.sublabel }}</p>
      </div>
    </div>
    ...
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        <h3 class="text-lg font-black text-slate-900 dark:text-white uppercase tracking-tighter">Recent Applications</h3>
        <router-link to="/employer/applications" class="text-[10px] font-black uppercase tracking-widest text-indigo-600 hover:text-indigo-700 transition">
      ...
      <div v-else-if="recentJobsWithApps.length === 0" class="text-center py-16 bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800">
        <i class="pi pi-inbox text-4xl text-slate-300 mb-4"></i>
        <p class="text-slate-400 font-bold uppercase text-xs tracking-widest">No applications yet.</p>
      </div>

      <div v-else class="space-y-4">
        <div v-for="job in recentJobsWithApps" :key="job.id"
          class="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 overflow-hidden hover:border-indigo-200 transition-all">
          <!-- Job Header -->
            ...
            <div class="flex items-center gap-4">
              ...
              <div>
                ...
                <div class="flex items-center gap-3 mt-1">
                  ...
                  <span class="text-[10px] font-bold text-slate-400">{{ job.applications_count || 0 }} applicant{{ job.applications_count === 1 ? '' : 's' }}</span>
                </div>
              </div>
            </div>
            <router-link :to="`/employer/jobs/${job.id}/applications`"
          ...
          <div class="divide-y divide-slate-50 dark:divide-slate-800">
            <router-link v-for="app in (job.applications || []).slice(0, 3)" :key="app.id"
            ...
            >
              <div class="flex items-center gap-4">
                ...
                />
                <div>
                  <p class="font-bold text-slate-900 dark:text-white text-sm">{{ app.candidate_snapshot?.name || 'Unknown' }}</p>
                  <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ app.candidate_snapshot?.headline || 'Candidate' }}</p>
                </div>
              </div>
              <div class="flex items-center gap-3">
                <span :class="statusClass(app.current_status)" class="text-[9px] font-black px-2 py-1 rounded-full uppercase">{{ app.current_status?.replace('_', ' ') }}</span>
                <span class="text-[10px] text-slate-400 font-bold">{{ formatDate(app.applied_at) }}</span>
              </div>
            </router-link>
    ...
    <div v-if="pipelineTotals.total > 0" class="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] border border-slate-100 dark:border-slate-800">
      <h3 class="text-xs font-black uppercase text-slate-400 tracking-widest mb-4">Pipeline Overview</h3>
...
const stats = computed(() => [
  ...
  {
    label: 'Shortlisted',
    value: (employerStore.applications || []).filter((a) => a.current_status === 'shortlisted').length,
    icon: 'pi pi-star',
    color: 'bg-amber-500',
  },
  {
...
const pipelineTotals = computed(() => {
  ...
  pipelineStages.forEach((s) => { totals[s.value] = 0 })
