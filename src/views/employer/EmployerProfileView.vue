<template>
  <div class="max-w-6xl mx-auto space-y-8 animate-fadeIn pb-20 px-4 md:px-0">
    ...
    <form @submit.prevent="handleSave" class="space-y-6">
      <!-- Hero / Cover Section -->
      <div
        class="relative rounded-[2.5rem] overflow-hidden border border-slate-100 dark:border-slate-800 shadow-xl"
      >
        <!-- Cover Image -->
        <div class="relative h-48 md:h-64 bg-slate-100 dark:bg-slate-800">
          ...
          <label
            class="absolute bottom-4 right-4 w-12 h-12 bg-white/90 dark:bg-slate-900/90 backdrop-blur hover:bg-indigo-600 hover:text-white text-slate-600 rounded-2xl flex items-center justify-center cursor-pointer shadow-lg transition-all duration-300 hover:rotate-12"
          >
        ...
        <div class="relative px-6 md:px-10 pb-8 -mt-16 flex flex-col md:flex-row items-end gap-6">
          ...
          <div class="relative group shrink-0">
            <div
              class="w-32 h-32 rounded-4xl overflow-hidden bg-white dark:bg-slate-900 border-[5px] border-white dark:border-slate-800 shadow-2xl"
            >
              <img
            ...
            <label
              class="absolute -bottom-2 -right-2 w-10 h-10 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl flex items-center justify-center cursor-pointer shadow-lg transition-all hover:rotate-12"
            >
          ...
          <div class="flex-1 pb-2">
            <div class="flex items-center gap-3 mb-1">
              ...
              <span
                ...
                class="bg-emerald-100 text-emerald-700 text-[10px] font-black px-2 py-1 rounded-full uppercase tracking-widest"
              >
                <i class="pi pi-verified mr-1"></i> Verified
              </span>
            </div>
            <div
              class="flex flex-wrap gap-3 text-sm text-slate-500 dark:text-slate-400 font-medium"
            >
              <span v-if="form.industry"
                ><i class="pi pi-briefcase mr-1"></i> {{ form.industry }}</span
              >
              <span v-if="form.company_size"
                ><i class="pi pi-users mr-1"></i> {{ form.company_size }}</span
              >
          ...
          <!-- Rating Badge -->
          <div
            v-if="form.average_rating > 0"
            class="flex items-center gap-1 bg-amber-50 dark:bg-amber-900/20 px-4 py-2 rounded-2xl border border-amber-100 dark:border-amber-900/30"
          >
            <i class="pi pi-star-fill text-amber-500"></i>
            <span class="font-black text-amber-700 dark:text-amber-400 text-lg">{{
              form.average_rating
            }}</span>
            <span class="text-xs text-amber-600 dark:text-amber-500 font-bold"
              >({{ form.total_reviews }} reviews)</span
            >
          </div>
        </div>
      </div>
      ...
      <!-- Identity Card -->
      <div
        class="bg-white dark:bg-slate-900 p-6 md:p-10 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-none"
      >
      ...
      >
        ...
        <div class="space-y-2">
          <textarea
            ...
            rows="6"
            placeholder="Tell us about your company vision, culture, and what makes you unique..."
            :class="
              errors.description
                ? 'ring-2 ring-rose-500/50 border-rose-500'
                : 'focus:ring-2 focus:ring-indigo-600/20 border-transparent'
            "
            class="w-full bg-slate-50 dark:bg-slate-800/50 border dark:border-slate-700 rounded-4xl px-6 py-4 text-sm font-bold text-slate-700 dark:text-slate-200 transition-all outline-none resize-none"
          ></textarea>
...
const validateForm = () => {
  ...
  if (
    ...
    form.cover_image.size > 5 * 1024 * 1024
  ) {
