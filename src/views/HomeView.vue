...
onMounted(async () => {
  if (authStore.isAuthenticated && authStore.user) {
    router.replace({ name: `${authStore.userRole}-dashboard` })
    return
  }
...
const goToCategory = (slug) => router.push({ name: 'jobs', query: { category: slug } })
  ...
  >
      ...
      >
        <transition-group name="fade" tag="div" class="absolute inset-0">
          ...
          >
            ...
            />
            <div
              class="relative z-20 w-full max-w-7xl mx-auto px-12 flex flex-col md:flex-row items-center justify-between gap-12"
            >
              <div class="max-w-3xl text-center md:text-left">
                <div
                  class="inline-flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-md border border-white/10 rounded-full mb-8"
                >
                  <span class="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em]">{{
                    slide.tag
                  }}</span>
                </div>
                <h1
                  class="text-6xl md:text-9xl font-black text-white tracking-tighter leading-[0.85] mb-8 italic"
                >
                  {{ slide.title.split(' ')[0]
                  }}<span class="text-indigo-500">{{ slide.title.split(' ')[1] }}</span>
                </h1>
                <p
                  class="text-xl md:text-2xl text-slate-300 mb-12 max-w-xl font-medium leading-relaxed"
                >
                  {{ slide.subtitle }}
                </p>
                <div class="flex flex-wrap gap-4 justify-center md:justify-start">
                  ...
                  >
                    <i class="pi pi-briefcase"></i> Join Elite
                  </button>
                </div>
              </div>
              <div
                class="hidden lg:block w-80 p-8 rounded-[2.5rem] bg-white/5 backdrop-blur-xl border border-white/10 shadow-2xl rotate-3 hover:rotate-0 transition-all duration-500"
              >
                <div class="flex items-center gap-4 mb-6">
                  <div
                    class="w-12 h-12 rounded-xl bg-indigo-600 text-white flex items-center justify-center font-black"
                  >
                    H
                  </div>
                  <div>
                    <p class="text-[10px] font-black uppercase text-indigo-400 tracking-widest">
                      Active Hiring
                    </p>
        ...
        <div class="absolute bottom-12 left-1/2 -translate-x-1/2 z-30 flex items-center gap-4">
          <button
            ...
            :class="[
              currentSlide === index ? 'bg-indigo-500 w-20' : 'bg-white/20 hover:bg-white/40',
            ]"
          ></button>
        </div>
      </section>
    ...
    <section class="max-w-[1800px] mx-auto px-10 py-20">
      ...
      <div class="flex gap-6 overflow-x-auto pb-8 snap-x no-scrollbar">
        <div
          v-if="taxonomyStore.loading"
          v-for="i in 4"
          :key="'skel-' + i"
          class="min-w-[280px] h-[250px] bg-slate-100 dark:bg-slate-900 rounded-[2.5rem] animate-pulse"
        ></div>
        ...
        >
          ...
          >
            <i
              :class="[getCategoryStyle(cat.icon).icon, getCategoryStyle(cat.icon).text]"
              class="text-3xl"
            ></i>
          </div>
          ...
          <p class="text-slate-400 font-bold text-xs uppercase tracking-widest">
            {{ cat.jobs_count || 0 }} Open Positions
          </p>
        </div>
      </div>
    </section>
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        >
          <div class="flex justify-between items-start mb-8">
            ...
            >
              <img
                :src="
                  job.employer?.logo ||
                  `https://ui-avatars.com/api/?name=${job.employer?.company_name}&background=6366f1&color=fff&bold=true`
                "
          ...
          <p class="text-slate-400 font-bold text-sm mb-8 flex items-center gap-2">
            <i class="pi pi-building text-xs"></i> {{ job.employer?.company_name }}
          </p>
          <div
          ...
          >
            <span class="text-lg font-black text-indigo-600 tracking-tight"
              ...
              <small class="text-[10px] opacity-60">{{ job.currency }}</small></span
            >
            <div
    ...
    <section class="max-w-[1800px] mx-auto p-10">
      ...
      >
        <div
          class="absolute -top-24 -right-24 w-96 h-96 bg-white/10 blur-[120px] rounded-full"
        ></div>
        <div
          class="absolute -bottom-24 -left-24 w-96 h-96 bg-black/10 blur-[100px] rounded-full"
        ></div>
        <div class="relative z-10 max-w-2xl mx-auto">
          ...
          <p class="text-indigo-100 text-lg md:text-xl font-medium mb-12 leading-relaxed">
            Join the elite network of Egyptian tech professionals and get access to the most
