<template>
  <div class="min-h-screen bg-[#FAFAFA] dark:bg-slate-950 font-['Outfit',sans-serif]">
    ...
    <div class="max-w-[1800px] mx-auto mb-8">
      <section
        class="relative overflow-hidden bg-slate-900 rounded-[3rem] py-14 px-12 shadow-2xl animate-[fadeIn_0.8s_ease-out]"
      >
        <div
          class="absolute -top-16 -right-16 w-64 h-64 bg-indigo-600/20 blur-[80px] rounded-full animate-pulse"
        ></div>
        <div class="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          <div
            class="flex items-center gap-6 bg-white/5 backdrop-blur-3xl p-6 rounded-[2rem] border border-white/10"
          >
            <div class="text-center px-6 border-r border-white/10">
              ...
              <p class="text-4xl font-black text-white italic tracking-tighter">
                {{ employerStore.totalApplications }}
              </p>
            </div>
            <div class="text-center px-6 border-r border-white/10">
              <p class="text-[10px] font-black text-amber-400 uppercase tracking-widest mb-1">
                Pending
              </p>
              <p class="text-4xl font-black text-white italic tracking-tighter">
                {{ employerStore.pendingApplications.length }}
              </p>
            </div>
            <div class="text-center px-6">
              ...
              <p class="text-4xl font-black text-white italic tracking-tighter">
                {{ employerStore.acceptedApplications.length }}
              </p>
            </div>
          </div>
        </div>
    ...
    <div class="max-w-[1800px] mx-auto">
        ...
        <button
          ...
          :class="[
            activeFilter === f.value
              ? filterStatusActiveStyle(f.value)
              : filterStatusInactiveStyle(f.value),
      ...
      <div v-else-if="filteredApplications.length === 0" class="text-center py-24">
        ...
        <h3 class="text-3xl font-black text-slate-900 dark:text-white italic tracking-tight mb-2">
          No applications here.
        </h3>
        <p class="text-slate-400 font-medium">
          {{
            activeFilter === 'all'
              ? "Candidates haven't applied yet."
        ...
        >
          <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8">
            ...
            <div class="flex items-start gap-6">
              ...
              <div>
                <h4
                  class="text-2xl font-black text-slate-900 dark:text-white tracking-tighter group-hover:text-indigo-600 transition-colors mb-1"
                >
                  {{ app.candidate_snapshot?.name || 'Candidate' }}
                </h4>
                <p
                  class="text-indigo-500 font-black text-[11px] uppercase tracking-widest italic mb-2"
                >
                  {{ app.candidate_snapshot?.email || '—' }}
                </p>
                <div class="flex flex-wrap items-center gap-3">
                  <span
                    class="inline-flex items-center gap-1.5 text-[11px] font-bold text-slate-400"
                  >
                    <i class="pi pi-briefcase text-indigo-400 text-xs"></i>
                    Applied for:
                    <span class="text-slate-700 dark:text-slate-300 font-black ml-1">{{
                ...
                <p
                  ...
                  class="text-slate-400 text-sm mt-3 leading-relaxed line-clamp-2 max-w-lg italic"
                >
                  "{{ app.cover_letter }}"
                </p>
                <button
                  @click="
                    $router.push({ name: 'employer.application-details', params: { id: app.id } })
                  "
...
const statusStyle = (status) => {
  ...
  if (s === 'accepted')
    return 'bg-emerald-50 text-emerald-600 border-emerald-100 dark:bg-emerald-500/10 dark:border-emerald-500/20 shadow-sm'
  if (s === 'rejected')
    return 'bg-rose-50 text-rose-600 border-rose-100 dark:bg-rose-500/10 dark:border-rose-500/20 shadow-sm'
...
const filterStatusActiveStyle = (status) => {
  if (status === 'all')
    return 'bg-slate-900 text-white shadow-xl shadow-slate-200 dark:shadow-none border-slate-900'
  if (status === 'accepted')
    return 'bg-emerald-500 text-white shadow-xl shadow-emerald-100 dark:shadow-none border-emerald-500'
  if (status === 'rejected')
    return 'bg-rose-500 text-white shadow-xl shadow-rose-100 dark:shadow-none border-rose-500'
  if (status === 'pending')
    return 'bg-amber-500 text-white shadow-xl shadow-amber-100 dark:shadow-none border-amber-500'
  return 'bg-indigo-600 text-white border-indigo-600'
}

const filterStatusInactiveStyle = (status) => {
  const common =
    'bg-white dark:bg-slate-900 transition-all border shadow-sm hover:translate-y-[-2px]'
  if (status === 'all')
    return `${common} border-slate-200 dark:border-slate-700 text-slate-600 hover:border-slate-400`
  if (status === 'accepted')
    return `${common} border-emerald-100 dark:border-emerald-500/20 text-emerald-600 hover:bg-emerald-50/50 dark:hover:bg-emerald-500/5`
  if (status === 'rejected')
    return `${common} border-rose-100 dark:border-rose-500/20 text-rose-500 hover:bg-rose-50/50 dark:hover:bg-rose-500/5`
  if (status === 'pending')
    return `${common} border-amber-100 dark:border-amber-500/20 text-amber-600 hover:bg-amber-50/50 dark:hover:bg-amber-500/5`
  return common
}
