<template>
  <div class="min-h-screen bg-[#FAFAFA] dark:bg-slate-950 font-['Outfit',sans-serif] pb-20">
    ...
    <div class="max-w-6xl mx-auto px-6 pt-12">
      ...
      <div
        v-if="loading"
        class="text-center py-24 text-slate-400 font-bold italic text-lg animate-pulse"
      >
      ...
      <div v-else-if="details" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1 space-y-6">
          <div
            class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] p-8 shadow-sm relative overflow-hidden group"
          >
            <div
              class="absolute top-0 right-0 w-32 h-32 bg-indigo-600/5 blur-[40px] rounded-full animate-pulse"
            ></div>

            <div class="relative z-10 text-center">
              <div
                class="w-24 h-24 bg-indigo-50 dark:bg-indigo-500/10 rounded-[2rem] flex items-center justify-center text-indigo-600 text-4xl font-black italic mx-auto mb-6 border border-indigo-100 dark:border-indigo-500/20 group-hover:bg-indigo-600 group-hover:text-white transition-all duration-500"
              >
              ...
              <p
                class="text-indigo-500 font-black text-[10px] uppercase tracking-widest italic mb-6"
              >
              ...
              <div class="flex flex-col gap-3">
                <div
                  class="flex items-center gap-3 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800"
                >
            ...
            <div class="mt-8 pt-8 border-t border-slate-50 dark:border-slate-800">
              <div class="flex items-center justify-between mb-2">
                ...
                >
                <span
                  :class="statusStyle(details.application.status)"
            ...
            <!-- Quick Actions -->
            <div
              v-if="details.application.status === 'pending'"
            ...
            <div
              v-else
              class="mt-8 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-dashed border-slate-200 dark:border-slate-700 text-center"
            >
              <p
                class="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-relaxed"
              >
                This application has already been
                <span :class="statusTextClass(details.application.status)">{{
                  details.application.status
                }}</span
                >.
              </p>
        ...
        <div class="lg:col-span-2 space-y-8">
          <!-- Cover Letter -->
          <section
            class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] p-10 shadow-sm animate-[fadeIn_0.6s_ease-out]"
          >
            <h3
              class="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-6 flex items-center gap-3"
            >
          ...
          <!-- Experience -->
          <section
            class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] p-10 shadow-sm animate-[fadeIn_0.7s_ease-out]"
          >
            <h3
              class="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-8 flex items-center gap-3"
            >
            ...
            <div v-else class="space-y-8 relative">
              <div
                class="absolute left-[20px] top-2 bottom-2 w-0.5 bg-slate-50 dark:bg-slate-800"
              ></div>
              <div v-for="exp in details.experience" :key="exp.id" class="relative pl-12">
                <div
                  class="absolute left-0 top-0 w-10 h-10 bg-white dark:bg-slate-900 border-2 border-slate-50 dark:border-slate-800 rounded-xl flex items-center justify-center z-10"
                >
                  <i class="pi pi-briefcase text-indigo-500 text-xs"></i>
                </div>
                <div>
                  <h4 class="text-lg font-black text-slate-900 dark:text-white tracking-tight">
                    {{ exp.title }}
                  </h4>
                  <div
                    class="flex items-center gap-2 text-indigo-500 font-bold text-[11px] uppercase tracking-widest mt-1"
                  >
          ...
          <!-- Education -->
          <section
            class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] p-10 shadow-sm animate-[fadeIn_0.8s_ease-out]"
          >
            <h3
              class="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-8 flex items-center gap-3"
            >
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              >
                <div class="flex items-center gap-3 mb-4">
                  <div
                    class="w-10 h-10 bg-indigo-50 dark:bg-indigo-500/10 rounded-xl flex items-center justify-center text-indigo-600"
                  >
                    <i class="pi pi-book"></i>
                  </div>
                  <h4 class="font-black text-slate-900 dark:text-white tracking-tight leading-none">
                    {{ edu.degree }}
                  </h4>
                </div>
                <p class="text-xs font-bold text-slate-500 mb-1">{{ edu.institution }}</p>
                <p class="text-[11px] font-black text-indigo-500 uppercase tracking-widest italic">
                  {{ edu.field_of_study }}
                </p>
                <p class="text-[10px] font-bold text-slate-400 mt-4">
          ...
          <!-- Skills -->
          <section
            class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] p-10 shadow-sm animate-[fadeIn_0.9s_ease-out]"
          >
            <h3
              class="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-6 flex items-center gap-3"
            >
            ...
            <div class="flex flex-wrap gap-3">
              ...
              >
                ...
                <span
                  class="text-[9px] font-black text-indigo-400 uppercase tracking-[0.2em] italic"
                  >{{ skill.level }}</span
                >
              </div>
              <div v-if="details.skills.length === 0" class="text-slate-300 italic text-sm">
                No skills listed.
              </div>
            </div>
          </section>
