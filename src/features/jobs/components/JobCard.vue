...
<template>
  <div
    class="group relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/60 p-7 rounded-3xl shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between overflow-hidden font-['Plus_Jakarta_Sans',sans-serif]"
  >
    <div
      class="absolute -top-24 -right-24 w-48 h-48 bg-sky-500/5 blur-[80px] group-hover:bg-sky-500/10 transition-colors duration-300"
    ></div>
    ...
    <div>
      <div class="flex justify-between items-start mb-8">
        ...
        >
          <span class="text-2xl font-black text-sky-700 dark:text-sky-300 uppercase italic">
            {{ job.company_name?.charAt(0) || 'J' }}
          </span>
        </div>

        <span
          class="px-4 py-2 bg-sky-50 dark:bg-sky-900/20 text-sky-700 dark:text-sky-300 text-[10px] font-black uppercase tracking-[0.2em] rounded-2xl border border-sky-100/50 dark:border-sky-700/30"
        >
          {{ job.type || 'Full Time' }}
        </span>
      </div>
      ...
      <h3
        class="text-2xl font-black text-slate-900 dark:text-white tracking-tighter leading-tight mb-3 group-hover:text-sky-700 dark:group-hover:text-sky-300 transition-colors duration-300"
      >
        {{ job.title }}
      </h3>
      ...
      <div class="flex flex-wrap gap-2 mb-10">
        ...
        >
          <i class="pi pi-wallet text-sky-600 text-[10px]"></i>
          <span
        ...
        <div
          class="flex items-center gap-1.5 px-4 py-2 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-transparent hover:border-slate-200 dark:hover:border-slate-700 transition-colors cursor-default"
        >
          <i class="pi pi-tag text-sky-600 text-[10px]"></i>
          <span
            class="text-[11px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-wider"
          >
            {{ job.category || 'Tech' }}
    ...
    <div class="flex items-center gap-3 pt-8 border-t border-slate-50 dark:border-slate-800/50">
      <button
        @click="handleApply"
        class="flex-1 py-4 bg-sky-600 text-white text-xs font-black uppercase tracking-[0.2em] rounded-[1.25rem] hover:bg-sky-700 active:scale-95 transition-all shadow-sm"
      >
        Apply
      </button>
