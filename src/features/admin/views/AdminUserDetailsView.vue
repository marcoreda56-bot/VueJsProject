      ...
      <div class="space-y-8">
        <div class="bg-white border border-gray-100 p-10 rounded-[3rem] shadow-sm text-center">
          <div class="relative inline-block mb-6">
            <div class="w-32 h-32 bg-gray-50 rounded-[2.5rem] overflow-hidden border-4 border-white shadow-xl mx-auto">
              <img :src="user.avatar || 'https://i.pravatar.cc/150?img=1'" alt="" class="w-full h-full object-cover" />
            </div>
            <div :class="['absolute bottom-2 right-2 w-6 h-6 border-4 border-white rounded-full shadow-sm', user.is_active ? 'bg-emerald-500' : 'bg-red-500']"></div>
          </div>
          
          <h1 class="text-2xl font-black text-slate-900 tracking-tight leading-tight">{{ user.name || 'Unnamed User' }}</h1>
          <p class="text-xs font-black text-gray-400 uppercase tracking-[0.2em] mt-2 mb-6">{{ user.role }} Account</p>
        ...
        <div class="bg-slate-900 p-8 rounded-[3rem] text-white relative overflow-hidden">
          ...
          <h3 class="font-black italic text-lg mb-6 relative z-10">Account History<span class="text-indigo-400">.</span></h3>
          <div class="space-y-4 relative z-10">
            ...
            <div>
             ...
             <div>
              <p class="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Email Verified</p>
              <p class="font-bold text-sm text-emerald-400">{{ user.email_verified_at ? formatDate(user.email_verified_at) : 'Not Verified' }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="lg:col-span-2 space-y-10">
        ...
        <div class="bg-white border border-gray-100 p-10 rounded-[3rem] shadow-sm">
          <div class="flex items-center gap-2 mb-8">
            ...
            <h2 class="text-xl font-black text-slate-900 italic">Profile Information<span class="text-indigo-600">.</span></h2>
          </div>

          <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
              <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Full Identity</p>
              <p class="font-bold text-slate-900">{{ user.name || 'N/A' }}</p>
            </div>
            <div>
              <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Communication</p>
            ...
            <div v-if="user.role === 'employer'">
              <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Organization</p>
              <p class="font-bold text-indigo-600">{{ profileData.company_name || 'N/A' }}</p>
            </div>
            <div v-if="user.role === 'candidate'">
              <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Professional Title</p>
              <p class="font-bold text-indigo-600">{{ profileData.title || 'N/A' }}</p>
            </div>
          </div>

          <div v-if="profileData.description" class="mt-8 pt-8 border-t border-gray-50">
            <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Description / Bio</p>
        ...
        <div v-if="user.role === 'employer'" class="bg-white border border-gray-100 rounded-[3rem] shadow-sm overflow-hidden">
          <div class="p-8 border-b border-gray-50 bg-gray-50/30">
            <h3 class="font-black text-slate-900 italic text-xl">Posted Jobs ({{ userJobs.length }})<span class="text-indigo-600">.</span></h3>
          </div>
          <div class="overflow-x-auto">
            <table class="w-full text-left">
              <thead>
                <tr class="bg-gray-50/50">
                  <th class="px-8 py-6 text-[10px] font-black text-gray-400 uppercase tracking-widest">Job Posting</th>
                  <th class="px-8 py-6 text-[10px] font-black text-gray-400 uppercase tracking-widest">Status</th>
                  <th class="px-8 py-6 text-[10px] font-black text-gray-400 uppercase tracking-widest text-right">Activity</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-50">
                <tr v-for="job in userJobs" :key="job.id" class="hover:bg-gray-50/50 transition-all group">
                  <td class="px-8 py-6">
                    <router-link :to="`/admin/management/jobs/${job.id}`" class="font-bold text-slate-900 hover:text-indigo-600 transition-colors">
                      {{ job.title }}
                    </router-link>
                  ...
                  <td class="px-8 py-6 text-right">
                    <span class="text-xs font-black text-indigo-600">{{ job.applications_count }} Apps</span>
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <div v-if="userJobs.length === 0" class="p-16 text-center">
             <p class="text-gray-300 font-black italic">This employer has not posted any jobs yet.</p>
          </div>
        </div>

        <!-- Case: Candidate (Applications) -->
        <div v-if="user.role === 'candidate'" class="bg-white border border-gray-100 rounded-[3rem] shadow-sm overflow-hidden">
          <div class="p-8 border-b border-gray-50 bg-gray-50/30">
            <h3 class="font-black text-slate-900 italic text-xl">Recent Applications ({{ userApplications.length }})<span class="text-indigo-600">.</span></h3>
          </div>
          <div class="overflow-x-auto">
            <table class="w-full text-left">
              <thead>
                <tr class="bg-gray-50/50">
                  <th class="px-8 py-6 text-[10px] font-black text-gray-400 uppercase tracking-widest">Position</th>
                  <th class="px-8 py-6 text-[10px] font-black text-gray-400 uppercase tracking-widest">Status</th>
                  <th class="px-8 py-6 text-[10px] font-black text-gray-400 uppercase tracking-widest text-right">Date</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-50">
                <tr v-for="app in userApplications" :key="app.id" class="hover:bg-gray-50/50 transition-all group">
                  <td class="px-8 py-6">
                    <router-link :to="`/admin/management/jobs/${app.job_id}`" class="font-bold text-slate-900 hover:text-indigo-600 transition-colors">
                      {{ getJobTitle(app.job_id) }}
                    </router-link>
                  </td>
                  <td class="px-8 py-6">
                    <span class="text-[10px] font-black uppercase bg-indigo-50 text-indigo-600 px-3 py-1 rounded-full">
                      {{ app.status }}
                    </span>
                  </td>
          ...
          <div v-if="userApplications.length === 0" class="p-16 text-center">
             <p class="text-gray-300 font-black italic">This candidate has not applied for any jobs yet.</p>
          </div>
        </div>
      </div>
    </div>
