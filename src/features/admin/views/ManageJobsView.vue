...
const updateJobStatus = async (jobId, status) => {
  ...
  const result = await Swal.fire({
    ...
    confirmButtonColor:
      status === 'active' ? '#4f46e5' : status === 'rejected' ? '#dc2626' : '#6b7280',
...
const submitJob = async () => {
  ...
  try {
    ...
    Swal.fire({
      title: 'Published!',
      text: 'Job listing has been created successfully.',
      icon: 'success',
      timer: 2000,
      showConfirmButton: false,
    })
  ...
  <div class="p-10 max-w-[1800px] mx-auto">
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col lg:flex-row gap-8 items-start">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-3 mb-4">
              <span
                :class="[
                  getStatusBadgeClass(job.status || 'active'),
              ...
              <span class="text-xs font-bold text-gray-400"
                >{{ job.type || 'Full Time' }} • {{ job.location || 'Remote' }}</span
              >
            </div>
            <router-link :to="`/admin/management/jobs/${job.id}`">
              <h2
                class="text-2xl font-black text-slate-900 tracking-tight mb-2 group-hover:text-indigo-600 transition-colors cursor-pointer"
              >
            ...
            <p class="text-gray-500 text-sm line-clamp-2 font-medium leading-relaxed">
          ...
          <!-- Actions -->
          <div
            class="lg:flex lg:flex-col lg:items-end gap-3 w-full lg:w-48 pt-4 lg:pt-0 border-t lg:border-t-0 border-gray-50"
          >
            <template v-if="job.status === 'pending'">
              ...
              >
                <i
                  :class="
                    isActionLoading(job.id, 'active') ? 'pi pi-spin pi-spinner' : 'pi pi-check'
                  "
                ></i>
                Approve
              </button>
              ...
              >
                <i
                  :class="
                    isActionLoading(job.id, 'rejected') ? 'pi pi-spin pi-spinner' : 'pi pi-times'
                  "
                ></i>
                Reject
              </button>
            ...
            <template v-else>
              ...
              >
                <i
                  :class="
                    isActionLoading(job.id, 'closed') ? 'pi pi-spin pi-spinner' : 'pi pi-lock'
                  "
                ></i>
                Close Job
              </button>
              ...
              >
                <i
                  :class="
                    isActionLoading(job.id, 'active') ? 'pi pi-spin pi-spinner' : 'pi pi-refresh'
                  "
                ></i>
                Re-activate
              </button>
            ...
            >
              <i
                :class="isActionLoading(job.id, 'delete') ? 'pi pi-spin pi-spinner' : 'pi pi-trash'"
              ></i>
              Delete
            </button>
          </div>
      ...
      <div
        ...
        class="p-20 text-center bg-white border border-gray-100 rounded-[2.5rem]"
      >
        <div
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      ></div>
      <div
        class="bg-white rounded-[3rem] w-full max-w-4xl max-h-[90vh] overflow-hidden shadow-2xl relative z-10 flex flex-col"
      >
        <div
          class="p-8 border-b border-gray-50 flex items-center justify-between bg-white sticky top-0 z-20"
        >
          <div>
            <h2 class="text-2xl font-black text-slate-900 italic">
              Post New Job<span class="text-indigo-600">.</span>
            </h2>
          ...
          <button
            @click="
              ...
              formErrors = []
            "
            class="w-12 h-12 rounded-2xl bg-gray-50 text-gray-400 hover:bg-red-50 hover:text-red-600 transition-all"
          >
        ...
        <div class="p-10 overflow-y-auto">
          ...
          <div
            ...
            class="mb-6 p-4 bg-red-50 rounded-2xl border border-red-100"
          >
            <p class="text-sm font-black text-red-600 mb-2">Please fix the following errors:</p>
            <ul class="list-disc list-inside space-y-1">
              <li v-for="err in formErrors" :key="err" class="text-sm text-red-500 font-medium">
                {{ err }}
              </li>
            </ul>
          </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                  ...
                  >
                    ...
                    <option v-for="emp in adminStore.employers" :key="emp.id" :value="emp.id">
                      {{ emp.company_name }}
                    </option>
                  </select>
                </div>
            ...
            <div class="space-y-6">
              <div class="grid grid-cols-2 gap-4">
                <div class="space-y-2">
                  ...
                  >
                  <input
                    v-model.number="newJob.salary_min"
                    type="number"
                    min="0"
                    class="w-full px-6 py-4 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-indigo-500/20 focus:bg-white transition-all font-bold text-slate-900"
                  />
                ...
                <div class="space-y-2">
                  ...
                  >
                  <input
                    v-model.number="newJob.salary_max"
                    type="number"
                    min="0"
                    class="w-full px-6 py-4 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-indigo-500/20 focus:bg-white transition-all font-bold text-slate-900"
                  />
              ...
              <div class="space-y-2">
                ...
                >
                <input
                  v-model.number="newJob.vacancies"
                  type="number"
                  min="1"
                  class="w-full px-6 py-4 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-indigo-500/20 focus:bg-white transition-all font-bold text-slate-900"
                />
            ...
            <div class="md:col-span-2 space-y-6 pt-4">
              ...
              <div class="space-y-2">
                ...
                <textarea
                  ...
                  class="w-full px-6 py-4 bg-gray-50 border-none rounded-[2rem] focus:ring-2 focus:ring-indigo-500/20 focus:bg-white transition-all font-bold text-slate-900 placeholder:text-gray-300 resize-none"
                ></textarea>
              </div>
              <div class="space-y-2">
                <label class="text-xs font-black text-slate-400 uppercase tracking-wider ml-1"
                ...
                <textarea
                  ...
                  rows="4"
                  placeholder="What are the essential skills?"
                  class="w-full px-6 py-4 bg-gray-50 border-none rounded-[2rem] focus:ring-2 focus:ring-indigo-500/20 focus:bg-white transition-all font-bold text-slate-900 placeholder:text-gray-300 resize-none"
                ></textarea>
              </div>
            </div>
