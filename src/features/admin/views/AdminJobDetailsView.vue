...
const employer = computed(
  () => adminStore.employers.find((e) => e.id == job.value.employer_id) || {},
)
  ...
  <div class="p-10 max-w-[1800px] mx-auto">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
      ...
      <div class="lg:col-span-2 space-y-10">
        <!-- Job Hero -->
        <div
          class="bg-white border border-gray-100 p-10 rounded-[3rem] shadow-sm relative overflow-hidden"
        >
        ...
        <div class="bg-white border border-gray-100 rounded-[3rem] shadow-sm overflow-hidden">
          <div class="p-8 border-b border-gray-50">
            <h3 class="font-black text-slate-900 italic text-xl">
              Applicants ({{ jobApplications.length }})<span class="text-indigo-600">.</span>
            </h3>
          </div>
              ...
              <tbody class="divide-y divide-gray-50">
                ...
                >
                  <td class="px-8 py-6">
                    <div class="flex items-center gap-3">
                      <div
                        class="w-8 h-8 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center text-xs font-black"
                      >
                        {{ getCandidateInfo(app.candidate_id).name.charAt(0) }}
                      </div>
                      <div>
                        <p class="font-bold text-slate-900 text-sm leading-none mb-1">
                          {{ getCandidateInfo(app.candidate_id).name }}
                        </p>
                        <p class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">
                          {{ getCandidateInfo(app.candidate_id).email }}
                        </p>
                      </div>
                    </div>
                  </td>
      ...
      <div class="space-y-10">
        ...
        <div class="bg-white border border-gray-100 p-8 rounded-[3rem] shadow-sm">
          ...
          <div class="text-center mb-8">
            <div
              class="w-20 h-20 bg-gray-50 rounded-3xl mx-auto mb-4 flex items-center justify-center border border-gray-100 overflow-hidden p-2"
            >
              <img
                v-if="employer.logo"
                :src="employer.logo"
            ...
            <p class="text-indigo-600 font-bold text-xs uppercase tracking-wider mt-1">
              {{ employer.industry }}
            </p>
          </div>
          ...
          <div class="space-y-4">
            ...
            <div class="flex items-center justify-between py-3 border-b border-gray-50">
              ...
              <a
                ...
                class="text-sm font-bold text-indigo-600 hover:underline overflow-hidden text-ellipsis"
                >{{ employer.website }}</a
              >
            </div>
          </div>
        ...
        <!-- Quick Stats -->
        <div
          class="bg-indigo-600 p-8 rounded-[3rem] shadow-xl shadow-indigo-100 text-white relative overflow-hidden"
        >
          <div
            class="absolute -right-4 -bottom-4 w-24 h-24 bg-white/10 rounded-full blur-2xl"
          ></div>
          <h3 class="font-black italic text-xl mb-6 relative z-10">
            Engagement Statistics<span class="text-indigo-300">.</span>
          </h3>
          <div class="space-y-6 relative z-10">
            ...
            <div>
              ...
              <p class="text-3xl font-black tracking-tight">
                {{ job.views ? ((jobApplications.length / job.views) * 100).toFixed(1) : 0 }}%
              </p>
            </div>
          </div>
        </div>
